...
int NDS_LoadROM( const char *filename, int bmtype, u32 bmsize, const char* logicalFilename)
#else
int NDS_LoadROM( const char *filename, int bmtype, u32 bmsize,
				const char *cflash_disk_image_file, const char* logicalFilename)
#endif
{
	int					ret;
	int					type;
...
bool NDS_ExportSave(const char *filename)
{
	...
	if (memcmp(filename+strlen(filename)-4, ".sav", 4) == 0) {
		...
		if(MMU.bupmem.size>0)
			fwrite((char*)MMU.bupmem.data,1,MMU.bupmem.size,outf);
		fclose(outf);
		return true;
	}
...
//this doesnt work anyway. why take a speed hit for public releases?
		...
		if(nds.cycles>=nds.nextHBlank)
		{
			...
			if(nds.cycles>=nds.nextHBlank+1092)
			{
				...
				//T1WriteWord(ARM9Mem.ARM9_REG, 0x1006, nds.VCount); // svn r2299
				T1WriteWord(MMU.ARM7_REG, 6, nds.VCount);
				//T1WriteWord(MMU.ARM7_REG, 0x1006, nds.VCount); // svn r2299
