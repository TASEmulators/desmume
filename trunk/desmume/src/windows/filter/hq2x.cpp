...
static u16 RGBtoBright[1<<NUMBITS];
...
void RenderHQ2X (SSurface Src, SSurface Dst)
{
    ...
    u8 *srcPtr = Src.Surface;
    u8 *dstPtr = Dst.Surface;
    u32 srcPitch = Src.Pitch;
    u32 dstPitch = Dst.Pitch;
    int width = Src.Width;
    int height = Src.Height;

	int	w1, w2, w3, w4, w5, w6, w7, w8, w9;

	u32	src1line = srcPitch >> 1;
	u32	dst1line = dstPitch >> 1;
	u16	*sp = (u16 *) srcPtr;
	u16	*dp = (u16 *) dstPtr;
...
/*			switch(GuiScale)
			...
			case FILTER_HQ2XBOLD: {
				const u16 avg = (RGBtoBright[w1] + RGBtoBright[w2] + RGBtoBright[w3] + RGBtoBright[w4] + RGBtoBright[w5] + RGBtoBright[w6] + RGBtoBright[w7] + RGBtoBright[w8] + RGBtoBright[w9]) / 9;
