...
+ (BOOL) loadState:(NSURL *)saveStateURL
{
	...
	const char *statePath = [[saveStateURL path] fileSystemRepresentation];
...
+ (BOOL) loadRom:(NSURL *)romURL
{
	...
	const char *romPath = [[romURL path] fileSystemRepresentation];
...
+ (BOOL) loadReplay:(NSURL *)replayURL
{
	...
	const char *replayPath = [[replayURL path] fileSystemRepresentation];
...
{
	BOOL result = NO;
	const char *romSavePath = [[romSaveURL path] fileSystemRepresentation];
...
+ (void) setupAllFilePathsForVersion:(NSString *)versionString port:(NSString *)portString
{
	...
	{
		strlcpy(path.pathToSounds, [[soundSamplesURL path] fileSystemRepresentation], MAX_PATH);
	}
	
	NSURL *firmwareURL = [CocoaDSFile directoryURLByKind:@"Firmware Configuration" version:versionString port:portString];
	if (firmwareURL != nil)
	{
		strlcpy(path.pathToFirmware, [[firmwareURL path] fileSystemRepresentation], MAX_PATH);
	}
	
	NSURL *luaURL = [CocoaDSFile directoryURLByKind:@"Lua Script" version:versionString port:portString];
	if (luaURL != nil)
	{
		strlcpy(path.pathToLua, [[luaURL path] fileSystemRepresentation], MAX_PATH);
	}
}
...
+ (NSString *) fileVersionByURL:(NSURL *)fileURL
{
	...
	versionURL = [CocoaDSFile userAppSupportURL:nil version:@"0.9.10"];
	versionPath = [versionURL path];
	if (versionPath != nil && [[[filePath stringByDeletingLastPathComponent] stringByDeletingLastPathComponent] isEqualToString:versionPath])
	{
		versionStr = @"0.9.10";
	...
	versionURL = [CocoaDSFile userAppSupportURL:nil version:@"0.9.11"];
	versionPath = [versionURL path];
	if (versionPath != nil && [[[filePath stringByDeletingLastPathComponent] stringByDeletingLastPathComponent] isEqualToString:versionPath])
	{
		versionStr = @"0.9.11";
	...
	versionURL = [CocoaDSFile userAppSupportURL:nil version:@"0.9.12"];
	versionPath = [versionURL path];
	if (versionPath != nil && [[[filePath stringByDeletingLastPathComponent] stringByDeletingLastPathComponent] isEqualToString:versionPath])
	{
		versionStr = @"0.9.12";
